...
while True:
    top3_list = [Beacon(0,-1),Beacon(0,-1),Beacon(0,-1)] #*수신된 비콘 주소, rssi 만 저장 #큐를 비운다.*
    receive_list =[]
    maclist = []
    ...
    for beacon in returnedList:
        ...
        if (now_mac in maclist) and (top3_list[maclist.index(now_mac)].getRSSSI() < now_rssi):#1. 중복이지만 새로 측정된 rssi가 더 크면 큐에 추가
                if len(top3_list) <3:
